...
import { CheckCircle2, AlertCircle, Server, Cpu, Zap, WifiOff, FileQuestion, ShieldAlert } from 'lucide-react';
...
export const SettingsPanel: React.FC<SettingsPanelProps> = ({ settings, onSave }) => {
  ...
  const runHealthCheck = async () => {
    ...
    const result = await checkLocalHealth(localSettings.localModelUrl, localSettings.localModelName);
    if (result.ok) {
  ...
  const renderErrorGuidance = (result: HealthCheckResult) => {
    ...
    switch (result.errorType) {
        ...
        case 'URL_INVALID':
            title = "Invalid URL";
            steps = ["Check the URL format. It usually ends in /api/generate (Ollama) or /v1/chat/completions."];
            break;
        case 'INVALID_FORMAT':
            title = "Unexpected Response";
            steps = ["The server replied, but the data wasn't what we expected. Check if the API endpoint is compatible with Ollama/OpenAI formats."];
            break;
        default:
            title = "Server Error";
            steps = [
    ...
    return (
        <div className="mt-4 p-4 bg-red-50 text-red-900 text-sm rounded-lg border border-red-100 animate-in fade-in slide-in-from-top-2">
            ...
            <div className="text-xs font-semibold uppercase tracking-wider text-red-500 mb-1">Troubleshooting Steps</div>
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 p-6">
      <div>
        ...
        <p className="text-slate-500 mt-1">Choose between cloud intelligence or local privacy.</p>
      </div>

      <div className="grid gap-6">
        {/* Provider Selection */}
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          <label className="text-sm font-semibold text-slate-700 uppercase tracking-wider mb-4 block">AI Provider</label>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            >
              <Cpu className={`w-8 h-8 mb-3 ${localSettings.provider === ModelProvider.GEMINI ? 'text-blue-600' : 'text-slate-400'}`} />
              <span className="font-semibold text-slate-800">Google Gemini</span>
              <span className="text-xs text-slate-500 mt-1">High Accuracy & Speed</span>
            </button>
            ...
            >
              <Server className={`w-8 h-8 mb-3 ${localSettings.provider === ModelProvider.LOCAL ? 'text-green-600' : 'text-slate-400'}`} />
              <span className="font-semibold text-slate-800">Local LLM</span>
              <span className="text-xs text-slate-500 mt-1">Privacy Focused (Ollama/LM Studio)</span>
            </button>
          </div>
        </div>
        ...
        {localSettings.provider === ModelProvider.GEMINI && (
          <div className="bg-blue-50/50 p-6 rounded-xl border border-blue-100">
             <div className="flex items-start gap-4">
                ...
                <div>
                   <h3 className="font-medium text-slate-800">Gemini Configuration</h3>
                   <div className="mt-4 flex items-center gap-3">
                      <label className="relative inline-flex items-center cursor-pointer">
                        ...
                        <span className="ml-3 text-sm font-medium text-slate-700">Enable Thinking Mode (Gemini 3.0 Pro)</span>
                      </label>
                   </div>
                   <p className="text-xs text-slate-500 mt-2">
                     Enables deep reasoning (Budget: 32k tokens) for complex style matching. Disable for faster responses via Flash Lite.
                   </p>
                </div>
             </div>
          </div>
        ...
        {localSettings.provider === ModelProvider.LOCAL && (
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
            <h3 className="font-medium text-slate-800">Local Model Connection</h3>
            
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Model Endpoint URL</label>
              <input
                type="text"
            ...
            <div className="pt-2">
              <div className="flex items-center gap-3">
                <Button variant="secondary" onClick={runHealthCheck} disabled={healthCheck.status === 'checking'}>
                   {healthCheck.status === 'checking' ? 'Checking...' : 'Test Connection'}
                </Button>
              ...
              {healthCheck.result && !healthCheck.result.ok && renderErrorGuidance(healthCheck.result)}
            </div>
          </div>
        )}
      </div>
