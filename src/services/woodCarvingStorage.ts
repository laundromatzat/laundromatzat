...
const DB_NAME = "laundromatzat-wood-carving";
...
export const isWoodCarvingStorageSupported = (): boolean =>
...
const openDatabase = async (): Promise<IDBDatabase> => {
  ...
  return await new Promise<IDBDatabase>((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(
        request.error ?? new Error("Failed to open wood carving storage.")
      );
  });
};
...
const runTransaction = async <T>(
  ...
  return await new Promise<T>((resolve, reject) => {
    ...
    if (req) {
      ...
      req.onerror = () =>
        fail(req.error ?? new Error("Wood carving storage request failed."));
    }

    tx.oncomplete = done;
    tx.onabort = () =>
      fail(tx.error ?? new Error("Wood carving storage transaction aborted."));
    tx.onerror = () =>
      fail(tx.error ?? new Error("Wood carving storage transaction failed."));
  });
};
...
export const loadProjects = async (): Promise<StoredProject[]> => {
  ...
  const records = await runTransaction<StoredProject[]>("readonly", (store) =>
    store.getAll()
  );
